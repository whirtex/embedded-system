...
A lista foi elaborada com base no objetivo do projeto e nos componentes disponíveis na faculdade. Este arquivo relaciona cada uma das seis referências selecionadas com a arquitetura, os requisitos e as especificações do sistema de monitoramento inteligente de colmeias.
...
As seis referências sustentam uma solução que combina sensores ambientais, coleta acústica ou vibroacústica, câmera na entrada, processamento local e comunicação sem fio. A literatura também indica que a arquitetura deve preservar os dados brutos, registrar o contexto das medições e permitir validação posterior, pois uma variável isolada não é suficiente para determinar a saúde da colônia.
